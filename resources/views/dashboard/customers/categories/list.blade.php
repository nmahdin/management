<x-admin.main title="لیست دسته‌بندی مشتریان">
            ...
            <div class="nk-content-body">
                <div class="nk-block-head nk-block-head-sm">
                    <div class="nk-block-between">
                        <div class="nk-block-head-content">
                            <!--   --------------- title --------------     -->
                            <h3 class="nk-block-title page-title">لیست دسته‌بندی مشتریان</h3>
                            <div class="nk-block-des text-soft">
                                <!--   --------------- توضیح صفحه --------------     -->
                                @if($n !== 0)
                                <p>در مجموع {{ $n }} دسته‌بندی وجود دارد.</p>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="toggle-wrap nk-block-tools-toggle">
                                ...
                                <div class="toggle-expand-content" data-content="pageMenu">
                                    <ul class="nk-block-tools g-3">
                                        <!--   --------------- links --------------     -->
                                        <li>
                                            <a href="{{ route('customers.categories.create') }}"
                                            ...
                                            <form id="form133" action="{{ route('customers.categories.create') }}"
                                                  class="d-none"></form>
                                        </li>
                                        <li>
                                            <a href="{{ route('customers.categories.trash') }}"
                                               class="dropdown-toggle btn btn-light btn-dim" data-bs-toggle="modal"
                                               data-bs-target="#modalZoom"
                                               onclick="event.preventDefault(); document.getElementById('cat_trash').submit();">
                                                <em class="icon ni ni-trash"></em>
                                                <span class="fw-normal">
                                                    سطل زباله
                                                </span>
                                            </a>
                                            <form id="cat_trash" action="{{ route('customers.categories.trash') }}"
                                                  class="d-none"></form>
                                        </li>
                                    </ul>
                                </div>
                ...
                @if(session('deleted'))
                    <div class="alert alert-fill alert-success alert-icon bg-success-dim text-success">
                        ...
                        <a href="{{ route('customers.categories.trash') }}" class="text-success" data-bs-toggle="modal"
                           data-bs-target="#modalZoom"
                           onclick="event.preventDefault(); document.getElementById('cat_trash').submit();">
                            <u>سطل زباله</u>
                        </a>
                ...
                @if(session('updated'))
                    <div class="alert alert-fill alert-success alert-icon bg-success-dim text-success">
                        <em class="icon ni ni-check-circle"></em>
                        دسته‌بندی "{{session('updated')}}" با موفقیت
                        ویرایش شد.
                    </div>
                @endif
                @if(session('error'))
            ...
            <!-- .nk-block-head -->
                @if($n == 0)
                    <div class="alert alert-fill alert-light alert-icon">
                        <em class="icon ni ni-alert-circle"></em>
                        هیچ دسته‌بندی مشتری وجود ندارد! سطل زباله را بررسی کنید.
                    </div>
                @endif
                            ...
                            <div class="card-inner">
                                <table class="table table-orders">
                                    ...
                                    <tr class="tb-odr-item">
                                        ...
                                        <th class="tb-odr-info">
                                            <span class="tb-odr-info">تعداد مشتریان</span>
                                        </th>
                                    ...
                                    @foreach($categories as $category)
                                        <tr class="tb-odr-item">
                                            <td class="tb-odr-info">
                                                <span class="tb-odr-info">{{ $category->category_name }}</span>
                                            </td>
                                            <td class="tb-odr-info">
                                                @if($category->notes)
                                            ...
                                            <td class="tb-odr-info">
                                                <span class="tb-odr-info">
                                                    {{ $category->customers->count() ?? 'error' }}
                                                </span>
                                            </td>
                                            <td class="tb-odr-action">
                                                <div class="tb-odr-btns d-none d-md-inline">
                                                    <a href="{{ route('customers.categories.edit', ['id' => $category->id]) }}" class="btn btn-warning btn-dim"><em
                                                ...
                                                <div class="tb-odr-btns d-none d-md-inline" style="margin-right: 3px">
                                                    <a href="{{ route('customers.categories.delete', ['id' => $category->id]) }}"
                                                    ...
                                                    <form id="delete_cat{{$category->id}}" method="post"
                                                          action="{{ route('customers.categories.delete', ['id' => $category->id]) }}"
