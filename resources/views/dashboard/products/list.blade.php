                            ...
                            <div class="toggle-wrap nk-block-tools-toggle">
                                ...
                                <div class="toggle-expand-content" data-content="pageMenu">
                                    <ul class="nk-block-tools g-3">
                                        <!--   --------------- links --------------     -->
                                        <li>
                                            <a href="{{ route('products.create') }}" class="dropdown-toggle btn btn-dark btn-dim " data-bs-toggle="modal" data-bs-target="#modalZoom"
                                               onclick="event.preventDefault(); document.getElementById('create').submit();">
                                                <em class="icon ni ni-plus"></em>
                                                <span class="fw-normal">
                                                    افزودن محصول
                                                </span>
                                            </a>
                                            <form id="create" action="{{ route('products.create') }}" class="d-none"></form>
                                        </li>
                                        <li>
                                            <a href="{{ route('products.trash') }}" class="dropdown-toggle btn btn-light btn-dim" data-bs-toggle="modal" data-bs-target="#modalTrash"
                                               onclick="event.preventDefault(); document.getElementById('trash').submit();">
                                                <em class="icon ni ni-trash"></em>
                                                <span class="fw-normal">
                                                    سطل زباله
                                                </span>
                                            </a>
                                            <form id="trash" action="{{ route('products.trash') }}" class="d-none"></form>
                                        </li>
                                    </ul>
                                </div>
                            </div>
...
{{--                                            </td>--}}
                                            ...
                                            <td class="nk-tb-col tb-col-sm" data-order="35040.34">
                                                <span class="fw-light"><a href="{{ route('products.detail' , ['product' => $product]) }}">{{ $product->product_id }}</a></span>
                                            </td>
                                            <td class="nk-tb-col tb-col-lg">
                                                <div class="user-card">
                                                    <div class="user-info">
                                                        <span class="tb-lead"><a href="{{ route('products.detail' , ['product' => $product]) }}">{{ $product->name }}</a><span class="dot dot-success d-md-none ms-1"></span></span>
                                            ...
                                            <td class="nk-tb-col nk-tb-col-tools">
                                                <ul class="nk-tb-actions gx-1">
                                                    <li class="nk-tb-action-hidden">
                                                        <a href="{{ route('products.detail' , ['product' => $product->id]) }}" class="btn btn-trigger btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="جزئیات">
                                                            <em class="icon ni ni-info-fill"></em>
                                                        </a>
                                                    </li>
                                                    <li class="nk-tb-action-hidden">
                                                        <a href="{{ route('products.edit' , ['product' => $product->id]) }}" class="btn btn-trigger btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="ویرایش">
                                                            <em class="icon ni ni-edit-alt-fill"></em>
                                                        </a>
                                                    </li>
                                                    <li class="nk-tb-action-hidden">
                                                        <a href="{{ route('products.delete' , ['product' => $product->id]) }}" onclick="event.preventDefault(); document.getElementById('delete_pro{{$product->id}}').submit();"
                                                           class="btn btn-trigger btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="حذف">
                                                            <em class="icon ni ni-trash-fill"></em>
                                                        </a>
                                                        <form id="delete_pro{{$product->id}}" method="post" action="{{ route('products.delete' , ['product' => $product->id]) }}" class="d-none">@csrf @method('delete')</form>
                                                    </li>
                                                    <li class="nk-tb-action-hidden">
                                                        <a href="{{ route('cart.add' , $product->id) }}" class="btn btn-trigger btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="افزودن به سبد خرید">
